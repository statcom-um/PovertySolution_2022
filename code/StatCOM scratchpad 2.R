...
prop_late_csv <- read.csv(file = "data/created_data/2022_11_22.csv")

prop_late_csv_orig <- read.csv(file = "data/created_data/2022_03_21.csv")
...
prop_late_csv$prop_of_income <- prop_late_csv$rent/prop_late_csv$income #rent as a proportion of income
prop_late_csv$prop_of_rent <- prop_late_csv$income/prop_late_csv$rent 
#some problems: get a 0 or -, but they have late payments?

prop_late_csv_2 = prop_late_csv %>% filter(prop_of_rent>1000000)
#these are people with rent=0, exclude from analysis bc should not have late payments

# model = logistic regression bc modeling proportion data
# outcome = proportion late
# predictor of interest = income as a proportion of rent
# other predictors = gender, age, num_hh, num_dep, building
# link function = binomial




prop_late_vs_building = ggplot(data=prop_late_csv, aes(x=name, y=proportion_late)) + geom_point()
prop_late_vs_building2 = ggplot(data=prop_late_csv, aes(x=name, y=proportion_late)) + geom_boxplot()
prop_late_vs_prop_income = ggplot(data=prop_late_csv, aes(x=prop_of_income, y=proportion_late)) + geom_point()
prop_late_vs_income = ggplot(data=prop_late_csv, aes(x=income, y=proportion_late)) + geom_point()
prop_late_vs_hhnum = ggplot(data=prop_late_csv, aes(x=num_hh, y=proportion_late)) + geom_point()
prop_late_vs_age = ggplot(data=prop_late_csv, aes(x=age, y=proportion_late)) + geom_point()
#age one, stratify/color by building
# treat household/number bedrooms as categorical
